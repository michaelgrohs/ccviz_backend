...
def upload_files():
    ...
    last_uploaded_data['bpmn_model'] = parse_bpmn(bpmn_path)
    if file_extension == '.csv':
        log_csv = pd.read_csv(xes_path, sep=None, encoding='utf-8-sig')
        last_uploaded_data['xes_log'] = log_converter.apply(log_csv)
    elif file_extension == '.xes':
        last_uploaded_data['xes_log'] = xes_importer.apply(xes_path)

    # Reset alignments cache
    last_uploaded_data['alignments'] = None

    return jsonify({
        "to_be_process": last_uploaded_data['bpmn_model'],
        "as_is_process": parse_xes(xes_path)  # assuming parse_xes returns structure you want in frontend
    })
